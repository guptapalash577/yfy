...
*)echo [Info] Connecting to mysql; echo [info] patch $SQLSCRIPT not installed.; mysql -u $username --password=$password $schema < $SQLSCRIPT; mysql -u $username --password=$password $schema -e "insert into tbl_installed_patches(patch_name)values('${SQLSCRIPT}');"
